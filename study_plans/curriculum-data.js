...
/* ===== Local persistence for the static administration panel ===== */
var DEFAULT_COLLEGES = JSON.parse(JSON.stringify(COLLEGES));
var STUDY_PLANS_STORAGE_KEY = 'moaeen-study-plans-v1';

function loadStudyPlans() {
  try {
    var saved = localStorage.getItem(STUDY_PLANS_STORAGE_KEY);
    return saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(DEFAULT_COLLEGES));
  } catch (error) {
    return JSON.parse(JSON.stringify(DEFAULT_COLLEGES));
  }
}

function saveStudyPlans(nextPlans) {
  COLLEGES = nextPlans;
  localStorage.setItem(STUDY_PLANS_STORAGE_KEY, JSON.stringify(COLLEGES));
}

function resetStudyPlans() {
  COLLEGES = JSON.parse(JSON.stringify(DEFAULT_COLLEGES));
  localStorage.removeItem(STUDY_PLANS_STORAGE_KEY);
}

COLLEGES = loadStudyPlans();
